...
### Understanding the limit behavior of Markov chain
# Example 1.3 Daily precipitation in San Francisco

P=matrix(c(418,256,256,884),2,2) ##wet to wet 418, wet to dry 256, dry to wet 256, dry to dry 884
P[1,]=P[1,]/sum(P[1,])  #convert the incident frequency to transition prob.
P[2,]=P[2,]/sum(P[2,])

P%*%P  #matrix multiplication
...
PI_sta=c(0.3715,0.6284)
PI_sta%*%P

## suppose initialize the chain by P(X_1=wet)=0.7, P(X_1=dry)=0.3
X0=c(0.7,0.3)
temp=X0
for (i in 1:10){
  temp=temp%*%P
...
## suppose initialize the chain by P(X_1=wet)=0.2, P(X_1=dry)=0.8
X0=c(0.2,0.8)
temp=X0
for (i in 1:10){
  temp=temp%*%P
...
## Sttionary distribution is determined by off-diagnonal entries in the transition matrix, 
## which defines the between state transition
...
mixture.dat = read.table("~/stat344F21/CompStat_book_dataset/mixture.dat",header=TRUE)
y = mixture.dat$y
...
##WHY SECOND PROPOSAL DOES NOT WORK WELL
##POSTERIAL SHAPE UNDER THE TWO PRIORS
d=seq(0.01,1,by=0.01)
#plot(d,sapply(d,f),type="l",ylab="posterior",xlab='dleta')
plot(d,dbeta(d,1,1),type="l")
plot(d,dbeta(d,2,10),type='l')
plot(d,sapply(d,f)/dbeta(d,1,1),type="l",ylab="f(d)/g(d)")  #plot wt
plot(d,sapply(d,f)/dbeta(d,2,10),type="l",ylab="f(d)/g(d)") #plot wt



############################################################################
### EXAMPLE 7.6 MIXTURE DISTRIBUTION, CONTINUED
############################################################################
...
## INITIAL VALUES
#READ IN THE DATA
mixture.dat = read.table("~/stat344F18/CompStat_book_dataset/mixture.dat",header=TRUE)
x = mixture.dat$y
...
#	R=exp(log.like(p[i+1],x)-log.like(p[i],x))*exp(u[i])/exp(u[i+1]) book might have errors
...
for (i in 1:(num.its-1)) {
  ...
  u[i+1]=u[i]+runif(1,-50,50)
	p2[i+1]=exp(u[i+1])/(1+exp(u[i+1]))
	R= exp(log.like(p2[i+1],x)-log.like(p2[i],x))*exp(u[i+1])/(1+exp(u[i+1]))^2/exp(u[i])*(1+exp(u[i]))^2
	if (R<1)
